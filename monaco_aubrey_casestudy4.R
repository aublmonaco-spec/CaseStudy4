...
#This step is reverting the names in the "farthest_JFK" data for clarity 
...
#For this map, I used geom_sf for the basemap and the "sf_airports" data, since they are spatial, then I used geom_point for the JFK_airport and farthest_JFK data. I specified for the point layers that the x-axis is based on longitude and the y-axis is based on latitude. I also added labels to the legend. Scale_color_identity allowed me to change the labels within the legend. 
airport_map <- ggplot() + geom_sf(data = world) + geom_sf(data = sf_airports, aes(color = "black")) + geom_point(data = JFK_airport, aes(x = lon, y = lat, color = "blue")) + geom_point(data = farthest_JFK, aes(x = lon, y = lat, color = "red")) + labs(title = "Global Airports With NYC Origins", x = "Longitude", y = "Latitude", colour = "Airport Type") + scale_color_identity(breaks = c("black", "blue", "red"), labels = c("Other Destinations", "JFK", "Farthest Airport"), guide = "legend")
